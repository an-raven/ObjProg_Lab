...
Studentas&Studentas::operator=(Studentas& rhs)
{
    Studentas temp(rhs);

    std::swap(vardas, temp.vardas);
    std::swap(pavarde, temp.pavarde);
    std::swap(egz, temp.egz);
    std::swap(nd, temp.nd);
...
//Studentas::Studentas(std::string vardas, std::string pavarde): Person(vardas, pavarde)// : vardas(vardas), pavarde(pavarde), nd(_nd), egz(_egz) {};{}
